...
function Sidebar({userData}) {
  ...
  return (
    <div style={{backgroundColor:"white",border:"1px solid #D6D6D6",width:"230px",height:"370px",borderRadius:"10px",marginLeft:"45px"}}>
      <img style={{height:"65px",width:"230px",borderTopRightRadius:"10px",borderTopLeftRadius:"10px"}} src={reactjs}/>
      <div style={{textAlign:"center"}}>  
      <img src={userData._document?.data?.value.mapValue.fields.profile_image.stringValue ?? profile} style={{width:"40px",borderRadius:"40px"}} />
      <h3 >{userData?._document?.data?.value.mapValue.fields.username.stringValue}</h3>
      {/* <h3>{userData && userData.username && userData.username.stringValue}</h3> */}
      </div>
      <h4 style={{textAlign:"center"}}>{userData?._document?.data?.value.mapValue.fields.designation.stringValue}</h4>
      <div style={{color:"#6F6F6F",borderTop:"1px solid #D6D6D6",paddingLeft:"10px"}}>


      <Link to="/connect" style={{textDecoration:"none",color:"grey"}} state={{username:userData._document?.data?.value.mapValue.fields.username.stringValue,
